...
const StoryCard: React.FC<{ item: (typeof successStoriesData)[number] }> = ({ item }) => (
  <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-card hover:shadow-card-hover transition-all duration-200 flex flex-col justify-between h-full">
    <div>
      ...
      <div className="flex items-center justify-between pt-2 pb-2.5 border-y border-slate-100 mb-3">
        <span className="text-sm font-black text-blue-700 font-heading">{item.packageOffered}</span>
          ...
          return (
            <>
              ...
              <div className="sm:hidden">
                <Carousel>
                  {[...successStoriesData.map((item) => <StoryCard key={item.id} item={item} />), ctaCard]}
                </Carousel>
              </div>
            </>
          );
