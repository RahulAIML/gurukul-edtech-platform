...
const ProjectCard: React.FC<{ project: (typeof featuredProjectsData)[number] }> = ({ project }) => (
  <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-card hover:shadow-card-hover hover:-translate-y-1 transition-all duration-200 flex flex-col justify-between h-full">
    <div>
      <div
      ...
      <p className="text-xs text-slate-600 leading-relaxed">{project.publicSummary}</p>
    </div>
    {project.technologies && (
      <div className="flex flex-wrap gap-1.5 mt-5 pt-4 border-t border-slate-50">
        {project.technologies.slice(0, 3).map((tech) => (
          <span key={tech} className="px-2 py-0.5 rounded-md text-[10px] font-semibold bg-slate-50 text-slate-600 border border-slate-100">
            {tech}
          </span>
        ))}
      </div>
  ...
  return (
    <section className="py-16 bg-white border-t border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Desktop / Tablet: 4 Projects Grid */}
